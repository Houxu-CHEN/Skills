...
% 用法：
% 1. 修改标题、日期。
% 2. 在首页汇总表中填入各类型条目、小计与总计。
% 3. 每个物品复制一段 \recordpage。
% 4. 有完整发票时用 \invoiceimage；缺票或未匹配时用 \invoiceplaceholder。
% 5. 最后一页 TODO 按实际缺口填写。
% 6. 这份模版默认可直接编译；示例页全部使用占位框，实际使用时再替换成真实图片路径。
...
\textbf{说明：}本版按“左上购买记录、右上付款记录、下方发票”的固定构图生成。首页为分类汇总表；后续每个物品单开一页；最后一页列出缺失或未匹配材料。

\recordpagecustom
{1. 示例物品 A}
{服装}
{¥88.00}
\topplaceholder{购买记录}{把这里替换成 \texttt{\string\topimage\{购买记录\}\{./path/to/order.png\}}}
\topplaceholder{付款记录}{把这里替换成 \texttt{\string\topimage\{付款记录\}\{./path/to/payment.png\}}}
...
\topplaceholder{购买记录}{把这里替换成 \texttt{\string\topimage\{购买记录\}\{./path/to/order.png\}}}
\topplaceholder{付款记录}{把这里替换成 \texttt{\string\topimage\{付款记录\}\{./path/to/payment.png\}}}
{\invoiceplaceholder{如已有完整发票，可改为 \texttt{\string\invoiceimage\{page=1\}\{./assets/example\_invoice.pdf\}}。}}
